...
$app->post('/api/QuickBooksAccounting/getExpensesByVendorReport', function ($request, $response, $args) {
    ...
    if (isset($post_data['args']['vendor']) && strlen($post_data['args']['vendors']) > 0) {
    ...
    try {
        $resp = $client->request('GET', 'company/' . $post_data['args']['companyId'] . '/reports/VendorExpenses', ['auth' => 'oauth', 'query' => $body]);
