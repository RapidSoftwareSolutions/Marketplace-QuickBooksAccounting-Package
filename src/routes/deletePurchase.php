...
$app->post('/api/QuickBooksAccounting/deletePurchase', function ($request, $response, $args) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'companyId', 'purchaseId', 'syncToken']);
...
//
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/purchase?operation=delete', ['auth' => 'oauth', 'json' => $body]);
