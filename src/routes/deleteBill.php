...
$app->post('/api/QuickBooksAccounting/deleteBill', function ($request, $response, $args) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'companyId', 'billId', 'syncToken']);
...
//
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/bill?operation=delete', ['auth' => 'oauth', 'json' => $body]);
