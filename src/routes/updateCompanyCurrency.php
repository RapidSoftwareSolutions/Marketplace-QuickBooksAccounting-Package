...
$app->post('/api/QuickBooksAccounting/updateCompanyCurrency', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'code', 'companyId', 'currencyId','syncToken']);
    ...
    $body['Id'] = $post_data['args']['currencyId'];
    ...
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/companycurrency', ['auth' => 'oauth', 'json' => $body]);
        $responseBody = $resp->getBody()->getContents();
        $rawBody = json_decode($resp->getBody());
        $all_data[] = $rawBody;

        if ($response->getStatusCode() == '200' && !isset($all_data[0]->Fault)) {
