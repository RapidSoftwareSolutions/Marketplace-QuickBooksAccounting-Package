...
$app->post('/api/QuickBooksAccounting/updateVendorcredit', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'vendorcreditLines', 'companyId', 'vendorRefId', "vendorcreditId"]);
    ...
    $body['Line'] = $post_data['args']['vendorcreditLines'];
    $body['Id'] = $post_data['args']['vendorcreditId'];
    ...
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/vendorcredit', ['auth' => 'oauth', 'json' => $body]);
