...
$app->post('/api/QuickBooksAccounting/deleteBillpayment', function ($request, $response, $args) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'companyId', 'billpaymentId', 'syncToken']);
...
//
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/billpayment?operation=delete', ['auth' => 'oauth', 'json' => $body]);
