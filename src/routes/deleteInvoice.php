...
$app->post('/api/QuickBooksAccounting/deleteInvoice', function ($request, $response, $args) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'companyId', 'invoiceId', 'syncToken']);
...
//
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/invoice?operation=delete', ['auth' => 'oauth', 'json' => $body]);
