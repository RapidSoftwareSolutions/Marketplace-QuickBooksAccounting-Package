...
$app->post('/api/QuickBooksAccounting/deleteSalesreceipt', function ($request, $response, $args) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'accessToken', 'tokenSecret', 'companyId', 'salesreceiptId', 'syncToken']);
...
//
    try {
        $resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/salesreceipt?operation=delete', ['auth' => 'oauth', 'json' => $body]);
