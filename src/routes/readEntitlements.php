...
$app->post('/api/QuickBooksAccounting/readEntitlements', function ($request, $response, $args) {
    ...
    $client = new GuzzleHttp\Client([
        ...
        'base_uri' => 'https://qbo.sbfinance.intuit.com/manage/',
        'handler' => $stack
    ]);
    try {
        $resp = $client->request('GET', 'entitlements/v3/' . $post_data['args']['companyId'], ['auth' => 'oauth', 'query'=> $body]);
        $responseBody = $resp->getBody()->getContents();
        $rawBody = json_decode($resp->getBody());
        $all_data[] = $rawBody;
        if ($response->getStatusCode() == '200' && !isset($rawBody->Fault)) {
