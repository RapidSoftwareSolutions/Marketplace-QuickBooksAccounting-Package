...
$app->post('/api/QuickBooksAccounting/updateCreditmemo', function ($request, $response, $args) {
    ...
    if (isset($post_data['args']['homeTotalAmt']) && strlen($post_data['args']['homeTotalAmt']) > 0) {
        $body['HomeTotalAmt'] = $post_data['args']['homeTotalAmt'];
    }
    ...
    var_dump($resp = $client->request('POST', 'company/' . $post_data['args']['companyId'] . '/creditmemo', ['auth' => 'oauth', 'json' => $body]));
    exit();
    try {
